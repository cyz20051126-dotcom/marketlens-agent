...
MarketLens Agent 是一个面向中国新茶饮/连锁咖啡品牌的轻量多智能体研究系统。它结合 Agentic RAG、网页研究、结构化证据库和金融分析视角，通过对话接收问题，先判断本地证据库是否足够；如果不够，就自动规划搜索任务、调用工具抓取公开资料、抽取结构化证据、复核来源质量；如果问题涉及金融分析，还会把经营证据转成单店模型、加盟模型和 DCF 假设，最后输出带引用的研究回答或报告。
...
v2 的目标是让项目真正具备 Agent runtime、工具调用、证据流和可观测性，而不是停留在 source-to-brief 报告生成。
...
9. 代码可以诚实体现 multi-agent research、tool calling、memory/trace、evidence extraction 和 finance analysis。
